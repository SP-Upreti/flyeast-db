...
export const blogApi = {
  ...
  getOne: (blogId: string) => api.get(`/blog/${blogId}`),
  getAllBlogs: (params?: { page?: number; limit?: number }) =>
    api.get("/blog", { params }).then((response) => {
      return {
  ...
  delete: (blogId: string) => api.delete(`/blog/${blogId}`),
...
export const usefulInfoAPi = {
  ...
  create: (data: object) => {
    console.log("usefulInfoAPi.create - Raw data being sent:", data);
    console.log(
      "usefulInfoAPi.create - Data stringified:",
      JSON.stringify(data, null, 2)
    );
    return api.post("/useful-info", data);
  },
  update: (id: string, data: object) => {
    console.log("usefulInfoAPi.update - Raw data being sent:", data);
    console.log(
      "usefulInfoAPi.update - Data stringified:",
      JSON.stringify(data, null, 2)
    );
    return api.put(`/useful-info/${id}`, data);
  },
  delete: (id: string) => api.delete(`/useful-info/${id}`),
};
...
export const teamMemberApi = {
  getAll: () => api.get("/teamMember"),
  create: (data: FormData) =>
    api.post("/teamMember", data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  getOne: (memberId: string) => api.get(`/teamMember/${memberId}`),
  update: (memberId: string, data: FormData) =>
    api.put(`/teamMember/${memberId}`, data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  delete: (memberId: string) => api.delete(`/teamMember/${memberId}`),
