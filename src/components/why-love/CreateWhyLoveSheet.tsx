...
import { useCreateWhyLove } from "@/hooks/useWhyLove";
...
export default function CreateWhyLoveSheet({
  onClose,
  ...
  const { mutateAsync: createWhyLove, isPending: isUpdating } =
    useCreateWhyLove();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createWhyLove(
        ...
        {
          onSuccess: () => {
            toast.success("Why Love created successfully");
            setFormData({
              title: "",
              description: "",
              isActive: true,
          ...
          onError: (error: any) => {
            console.error("Failed to create why love:", error);
            toast.error("Failed to create why love");
          },
        }
      );
    } catch (error) {
  ...
  return (
    <div className="p-6 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">Create Why Love</h1>
          <p className="text-muted-foreground">
            Add a new why love for this package
          </p>
        </div>
        <Button
                              onClick={handleClose}
