...
export default function PackageListPage({ onClose }: { onClose: () => void }) {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      if (values.season && Array.isArray(values.season)) {
        values.season.forEach((seasonItem: string, index: number) => {
          formData.append(`season[${index}]`, seasonItem);
        });
      }
