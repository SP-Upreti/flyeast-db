...
import { useCreateInsurance } from "@/hooks/useInsurance";
...
interface CreateInsuranceSheetProps {
  id: string;
  onClose?: () => void;
}

export default function CreateInsuranceSheet({
  onClose,
}: CreateInsuranceSheetProps) {
  ...
  const { mutateAsync: createInsurance, isPending: isUpdating } =
    useCreateInsurance();
  ...
  return (
    <div className="p-6 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">Create Insurance</h1>
          <p className="text-muted-foreground">
            Add a new insurance for this package
          </p>
        </div>
        <Button
                              onClick={handleClose}
