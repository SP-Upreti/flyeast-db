...
import { useUpdateInsurance, useGetInsuranceById } from "@/hooks/useInsurance";
...
export default function EditImportantNoticeSheet({
  ...
  const { mutateAsync: updateInsurance, isPending: isUpdating } =
    useUpdateInsurance(id);
  const { data: requirementData } = useGetInsuranceById(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateInsurance(formDataToSend, {
        onSuccess: () => {
  ...
  return (
    <div className="p-6 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">Edit Insurance</h1>
          <p className="text-muted-foreground">
            Update insurance details for this package
          </p>
        </div>
        <Button
                              onClick={handleClose}
