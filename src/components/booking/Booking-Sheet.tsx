...
const InvoiceComponent = ({
  ...
  const handlePrint = () => {
    try {
      ...
      console.log("printJS called successfully");
      toast.success("Print dialog opened!");
    } catch (error) {
      console.error("Print-js failed with error:", error);
...
>
  ...
  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; padding: 1.5rem; border-bottom: 1px solid #e5e7eb;">
    ...
    <div style="display: flex; justify-content: flex-end;">
      <img src="/logo/logo2.png" alt="real himalaya" style="width: 11rem;" />
    </div>
  </div>
  ...
  const generatePdfProgrammatically = async () => {
    try {
      ...
      // ---------- HEADER ----------
      try {
        const logo = await loadImageAsBase64("/logo/logo2.png");
        if (logo) {
  ...
  // Simplified PDF generation without images as fallback
  const generateSimplePdf = async () => {
    ...
    pdf.setFontSize(14);
    pdf.text("Real-Himalaya Adventures Pvt. Ltd.", 15, yPosition + 10);
    ...
    pdf.text(`Invoice #: ${bookingData.bookingReference}`, 15, yPosition);
    pdf.text(`Date: ${format(new Date(), "MMM d, yyyy")}`, 15, yPosition + 5);
    yPosition += 23;

    // Company info
    pdf.text("From:", 15, yPosition);
    pdf.text("Real-Himalaya Adventures Pvt. Ltd.", 15, yPosition + 5);
    pdf.text("Pulchowk, Lalitpur, Nepal", 15, yPosition + 10);
    pdf.text("support@Real-Himalaya.com", 15, yPosition + 15);
    yPosition += 33;

    // Client info
    const user = bookingData.personalInfo[0] || {};
    pdf.text("Bill To:", 15, yPosition);
    pdf.text(user.fullName || "N/A", 15, yPosition + 5);
    if (user.email) pdf.text(user.email, 15, yPosition + 10);
    if (user.country) pdf.text(user.country, 15, yPosition + 15);
    yPosition += 33;

    // Package details
    pdf.text("Package Details:", 15, yPosition);
    pdf.text(bookingData.package?.name || "Adventure Package", 15, yPosition + 5);
    pdf.text(`Duration: ${bookingData.package?.duration || "N/A"}`, 15, yPosition + 10);
    pdf.text(`Travelers: ${quantity}`, 15, yPosition + 15);
    yPosition += 33;

    // Pricing
    pdf.text("Invoice Items:", 15, yPosition);
    yPosition += 13;

    // Table configuration for simplified PDF
    const tableStartY = yPosition;
    const tableWidth = 150;
    const tableStartX = 15;
    const rowHeight = 8;
    const colWidths = [50, 20, 30, 30]; // Description, Qty, Unit Price, Total
    const colStartX = [15, 65, 85, 115]; // Column start positions
    ...
    pdf.setFontSize(9).setFont("helvetica", "bold");
    pdf.text("Description", colStartX[0] + 2, yPosition + 5);
    pdf.text("Qty", colStartX[1] + 2, yPosition + 5);
    pdf.text("Unit Price", colStartX[2] + 2, yPosition + 5);
    pdf.text("Total", colStartX[3] + 2, yPosition + 5);
    yPosition += rowHeight;

    // Data row with borders
    pdf.setFontSize(9).setFont("helvetica", "normal");
    const packageName = bookingData.package?.name || "Adventure Package";
    const maxNameLength = 20; // Adjust based on column width
    const displayName = packageName.length > maxNameLength
      ? packageName.substring(0, maxNameLength - 3) + "..."
      : packageName;

    pdf.text(displayName, colStartX[0] + 2, yPosition + 5);
    pdf.text(quantity.toString(), colStartX[1] + 2, yPosition + 5);
    pdf.text(`${currencySymbol}${unitPrice.toFixed(2)}`, colStartX[2] + colWidths[2] - 2, yPosition + 5, { align: "right" });
    pdf.text(`${currencySymbol}${subtotal.toFixed(2)}`, colStartX[3] + colWidths[3] - 2, yPosition + 5, { align: "right" });
    yPosition += rowHeight + 15; // Add extra spacing after table

    // Totals
    pdf.text(`Subtotal: ${currencySymbol}${subtotal.toFixed(2)}`, 110, yPosition);
    pdf.text(`PAX Discount (${paxDiscountPercent}%): -${currencySymbol}${discountAmount.toFixed(2)}`, 110, yPosition + 5);
    pdf.setFont("helvetica", "bold");
    pdf.text(`Total: ${currencySymbol}${total.toFixed(2)}`, 110, yPosition + 15);

    // Generate filename and save
    const fileName = `Real-Himalaya_Invoice_${bookingData.bookingReference}_${format(new Date(), "yyyy-MM-dd")}.pdf`;
    pdf.save(fileName);
  };
  ...
  return (
    <>
      ...
      {/* <div className="w-full mb-4 p-4 bg-red-50 border border-gray-200 rounded-sm no-print">
        <div className="flex items-start space-x-3">
          <Icon icon="solar:info-circle-bold" width="20" height="20" className="text-red-500 mt-0.5" />
          <div>
            <h4 className="font-semibold text-red-900 mb-1">Download Options</h4>
      ...
      <div className="w-full flex flex-wrap justify-between items-center gap-4 mb-6 p-4 bg-gray-50 rounded-sm border border-gray-200 no-print">
        ...
        <div className="">
          ...
          <div className="flex gap-3 items-center no-print">
            ...
            >
              <SelectTrigger className="w-full max-w-xs bg-white border-gray-300 hover:border-gray-400 focus:border-red-500 rounded-sm">
      ...
      >
        ...
        <div className="grid border-b grid-cols-2 gap-6 p-6 ">
          ...
          <div className="flex justify-end">
            <img src="/logo/logo2.png" alt="real himalaya" className="w-44" />
          </div>
        </div>
        {/* Company and Client Info */}
