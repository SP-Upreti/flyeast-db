...
export const useCreateUsefulInfo = () => {
  ...
  return useMutation({
    mutationFn: async (data: { name: string; description: string }) => {
      console.log("useCreateUsefulInfo - Data being sent to API:", data);
      console.log(
        "useCreateUsefulInfo - Description content:",
        data.description
      );
      const res = await usefulInfoAPi.create(data);
      console.log("useCreateUsefulInfo - API response:", res.data);
...
export const useUpdateUsefulInfo = (id: string) => {
  ...
  return useMutation({
    mutationFn: async (data: { name: string; description: string }) => {
      console.log("useUpdateUsefulInfo - Data being sent to API:", data);
      console.log(
        "useUpdateUsefulInfo - Description content:",
        data.description
      );
      const res = await usefulInfoAPi.update(id, data);
      console.log("useUpdateUsefulInfo - API response:", res.data);
      return res.data.data;
    },
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["useful-info"] }),
